...
from lueur.platform.gcp.zone import list_project_zones, list_project_region_zones
...
@respx.mock
@pytest.mark.asyncio
async def test_list_project_region_zones(gcp_project: str, gcp_region: str) -> None:
    respx.post(
        "https://oauth2.googleapis.com/token",
    ).respond(
        content=msgspec.json.encode({
    ...
    assert await list_project_region_zones(gcp_project, gcp_region) == ["us-east1-a"]
