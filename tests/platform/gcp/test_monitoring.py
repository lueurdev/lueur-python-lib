...
async def test_explore_monitoring(
    ...
    respx.get(
        ...
        content=msgspec.json.encode({"serviceLevelObjectives": [gcp_monitoring_slo[0]]})
    )
    
    respx.get(
        f"https://monitoring.googleapis.com/v3/projects/{gcp_project_number}/services/demo-service/serviceLevelObjectives",
    ).respond(
        content=msgspec.json.encode({"serviceLevelObjectives": [gcp_monitoring_slo[1]]})
    )
    
    respx.get(
        f"https://monitoring.googleapis.com/v3/projects/{gcp_project}/alertPolicies",
    ).respond(
        content=msgspec.json.encode({"alertPolicies": gcp_monitoring_alert_policies})
    )
    
    respx.get(
        f"https://monitoring.googleapis.com/v3/projects/{gcp_project}/notificationChannels",
    ).respond(
        content=msgspec.json.encode({"notificationChannels": gcp_monitoring_notification_channels})
    )
    
    respx.get(
        f"https://monitoring.googleapis.com/v3/projects/{gcp_project}/groups",
    ).respond(
        content=msgspec.json.encode({"group": []})
    )
